...
// Helper to sanitize Supabase URL (strip trailing slashes, quotes, or accidental path suffixes like /auth/v1)
export const sanitizeSupabaseUrl = (urlStr) => {
  if (!urlStr) return '';
  let cleaned = urlStr.trim().replace(/^["']|["']$/g, '');
  // Strip trailing path suffixes if full API endpoint URL was pasted
  cleaned = cleaned.replace(/\/(auth|rest|storage|graphql)(\/v\d+)?\/?$/i, '');
  // Strip all trailing slashes
  cleaned = cleaned.replace(/\/+$/, '');
  return cleaned;
};

export const sanitizeSupabaseKey = (keyStr) => {
  if (!keyStr) return '';
  return keyStr.trim().replace(/^["']|["']$/g, '');
};
...
const rawStoredUrl = typeof window !== 'undefined' ? (localStorage.getItem('supabase_url') || '') : '';
const rawStoredKey = typeof window !== 'undefined' ? (localStorage.getItem('supabase_anon_key') || '') : '';

export const SUPABASE_URL = sanitizeSupabaseUrl(rawStoredUrl || envUrl);
export const SUPABASE_ANON_KEY = sanitizeSupabaseKey(rawStoredKey || envKey);
